-- Security-MO Airport Database Initialization
-- SQL Server Express Schema and Data
...
BEGIN
    ...
    CREATE INDEX IX_Airports_Name ON Airports(name);
END

-- Insert comprehensive airport data
IF NOT EXISTS (SELECT * FROM Airports WHERE code = 'MNL')
BEGIN
...
-- Create a stored procedure for airport search
IF EXISTS (SELECT * FROM sys.procedures WHERE name = 'SearchAirports')
    DROP PROCEDURE SearchAirports;
GO

CREATE PROCEDURE SearchAirports
...
BEGIN
    ...
    ORDER BY 
        CASE 
            WHEN code LIKE @SearchTerm + '%' THEN 1
...
PRINT 'Airport database initialization completed successfully!';
